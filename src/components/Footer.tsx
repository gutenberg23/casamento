...
export const Footer: React.FC<FooterProps> = ({ onOpenAdmin, onOpenCheckup }) => {
  return (
    <footer className="py-12 px-6 bg-[#FCF9F3] border-t border-[#3A2E22]/10 text-center text-xs text-[#7A6A57]">
      <div className="max-w-4xl mx-auto flex flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-[#3A2E22]/10">
          <button
            onClick={onOpenCheckup}
            className="text-[11px] text-[#5C6748] hover:underline cursor-pointer"
          >
            Check-up do Sistema
          </button>
          <span className="text-[#3A2E22]/20">·</span>
          <button
            onClick={onOpenAdmin}
